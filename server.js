...
setInterval(() => {
    ...
    for (let id in players) {
        ...
        if(d > 5) { 
            let s = (p.isBoosting?18:6.5)*Math.pow(p.radius/INITIAL_RADIUS, -0.15); 
            p.x += Math.cos(a)*(d<50?s*d/50:s); p.y += Math.sin(a)*(d<50?s*d/50:s); 
        }
    ...
    for(let pid in players) {
        ...
        let visible = {};
        for(let oid in players) {
            if(Math.abs(me.x - players[oid].x) < 3500 && Math.abs(me.y - players[oid].y) < 2500) visible[oid] = players[oid];
        }
